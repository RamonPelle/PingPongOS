...
task_t *sleep_q = NULL;   // fila de tarefas adormecidas 
...
void dispatcher() {
    ...
    while (user_tasks > 0) {
        ...
        if (next_task) {
            ...
            if (next_task->status != TERMINADA && next_task->status != SUSPENSA) {
                queue_append((queue_t **)&ready_q, (queue_t *)next_task);
            }
            else if (next_task->status == TERMINADA) {
...
void task_sleep(int t) {
    ...
    // Desabilita preempção para manipulação de fila
    is_preempcao = 0; 

    // Calcula o tempo de acordar e armazena na tarefa
    current_task->wakeup_time = systime() + (unsigned int)t;

    #ifdef DEBUG
    printf("task_sleep: tarefa %d dormindo por %dms, acordar em %u\n", current_task->id, t, current_task->wakeup_time);
    #endif

    // Suspende a tarefa na fila de adormecidas
    task_suspend(&sleep_q); 

    // Reabilita a preempção (após a tarefa voltar do suspend)
    is_preempcao = 1;
}
...
void check_sleep_queue() {
    ...
    // Desabilita preempção para varrer a fila de adormecidas
    is_preempcao = 0; 

    unsigned int time = systime();
    task_t *task = sleep_q;

    int n = queue_size((queue_t *)sleep_q);

    for (int i = 0; i < n; i++) {
        task_t *next = task->next;
        
        // Se o tempo de acordar já passou
        if (time >= task->wakeup_time) {
            #ifdef DEBUG
            printf("dispatcher: acordando tarefa %d (wakeup: %u, now: %u)\n", task->id, task->wakeup_time, time);
            #endif
            
            // task_awake remove de sleep_q e coloca em ready_q
            task_awake(task, &sleep_q); 
        }
        
        task = next;
        
        // Se a fila esvaziou durante a varredura, paramos
        if (!sleep_q) break; 
    }
